...
func _normal_block(state: GameState, attackers: Array) -> Dictionary:
	...
	var my_nexus = state.get_player(AI_PID).nexus_hp
	...
	# --- Bước 1: Ngăn lethal hoặc giảm damage nặng ---
	# Nếu damage > 20% HP hiện tại hoặc sắp chết, phải cực kỳ ưu tiên block
	var high_priority = (total_unblocked_dmg >= my_nexus) or (total_unblocked_dmg >= 4)
	
	# --- Bước 2: Tìm trade tốt nhất cho từng attacker ---
	# Sắp xếp attackers theo damage giảm dần để ưu tiên block con mạnh nhất
	var sorted_attackers = attackers.duplicate()
	sorted_attackers.sort_custom(func(a, b):
		var ua = state.get_unit_by_uid(a)
		var ub = state.get_unit_by_uid(b)
		if ua == null or ub == null: return false
		return ua.get_effective_attack() > ub.get_effective_attack()
	)
	
	for atk_uid in sorted_attackers:
		if available_uid.is_empty(): break
		var attacker = state.get_unit_by_uid(atk_uid)
		if attacker == null: continue
		
		var best_blocker = _find_best_blocker_smart(attacker, available_uid, state, high_priority)
		if best_blocker != -1:
			assignments[atk_uid] = best_blocker
			available_uid.erase(best_blocker)
	
	return assignments

func _find_best_blocker_smart(attacker: UnitInstance, available: Array, state: GameState, high_priority: bool) -> int:
	var atk_dmg = attacker.get_effective_attack()
	var atk_hp = attacker.health
	
	# 1. Tìm trade thắng: mình sống, nó chết
	for uid in available:
		var b = state.get_unit_by_uid(uid)
		if b.get_effective_attack() >= atk_hp and b.health > atk_dmg:
			return uid
			
	# 2. Tìm trade hòa: cả hai chết (ưu tiên dùng unit rẻ/không phải champ)
	for uid in available:
		var b = state.get_unit_by_uid(uid)
		if b.get_effective_attack() >= atk_hp:
			# Chấp nhận trade hòa nếu nó mạnh (atk >= 2) hoặc mình đang bị ép sân
			if atk_dmg >= 2 or high_priority or not b.is_champion:
				return uid

	# 3. Chặn để giảm damage (mình sống, nó sống - như trường hợp 1/2 vs 1/2)
	for uid in available:
		var b = state.get_unit_by_uid(uid)
		if b.health > atk_dmg:
			# Chấp nhận chặn nếu giảm được ít nhất 1 damage và mình không chết
			if atk_dmg >= 1:
				return uid
				
	# 4. Hy sinh (mình chết, nó sống) - Chỉ làm nếu sắp thua (high_priority)
	if high_priority:
		for uid in available:
			var b = state.get_unit_by_uid(uid)
			if not b.is_champion: # Không hy sinh champion vô ích
				return uid
