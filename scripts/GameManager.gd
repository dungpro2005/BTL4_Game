...
var state: GameState
var ai: AIPlayer
var ai_difficulty: int = AIPlayer.Difficulty.NORMAL
...
#   GAME SETUP
# ============================================================
func start_new_game(difficulty: int = AIPlayer.Difficulty.NORMAL):
...
func player_declare_attack(attacker_uids: Array):
	...
	# AI chọn block
	var block_map = ai.decide_blockers(state, attacker_uids)
...
func _run_ai_turn():
	...
	var action = ai.decide_action(state)
	...
	match action.get("type", "pass"):
		...
		"declare_attack":
			...
			if not attackers.is_empty():
				...
				emit_signal("log_message", "AI declare attack với %d unit!" % attackers.size())
				emit_signal("state_updated", state)
				
				await get_tree().create_timer(0.8).timeout
				# Player chọn block → UI xử lý, set waiting_for_player_block = true
				state.phase = GameState.Phase.COMBAT_BLOCK
